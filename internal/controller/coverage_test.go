...
func TestSummarize(t *testing.T) {
	drifted, summary := summarize([]*diff.Result{
		{APIVersion: "v1", Kind: kindConfigMap, Name: "a", Namespace: nsDefault, Status: diff.StatusChanged},
		{APIVersion: "v1", Kind: kindService, Name: "b", Namespace: nsDefault, Status: diff.StatusNew},
		{APIVersion: "apps/v1", Kind: "Deployment", Name: "c", Namespace: nsDefault, Status: diff.StatusDeleted},
		{APIVersion: "v1", Kind: "Secret", Name: "d", Namespace: nsDefault, Status: diff.StatusUnchanged},
	})
	...
	if drifted[0].Kind != kindConfigMap || drifted[0].Status != myv1.DriftChanged {
...
func TestResolveGitAuth_BasicMissingUsername(t *testing.T) {
	scheme := newScheme(t)
	sec := gitAuthSecret(secretNameCreds, map[string][]byte{secretKeyPassword: []byte(testToken)}) // no username
...
func TestResolveGitAuth_BearerMissingToken(t *testing.T) {
	scheme := newScheme(t)
	sec := gitAuthSecret(secretNameCreds, map[string][]byte{}) // no bearerToken
	r := reconcilerFor(scheme, &fakeFetcher{}, sec)
	if _, err := r.resolveGitAuth(context.Background(), nsDefault,
		&myv1.GitAuth{Type: myv1.GitAuthBearer, SecretRef: myv1.LocalSecretRef{Name: secretNameCreds}}); err == nil {
...
func TestResolveGitAuth_SSHMissingIdentity(t *testing.T) {
	scheme := newScheme(t)
	sec := gitAuthSecret(secretNameCreds, map[string][]byte{"known_hosts": []byte("github.com ssh-ed25519 AAAA")}) // no identity
	r := reconcilerFor(scheme, &fakeFetcher{}, sec)
	if _, err := r.resolveGitAuth(context.Background(), nsDefault,
...
func TestResolveWebhookURL_KeyNotFound(t *testing.T) {
	...
	r := reconcilerFor(scheme, &fakeFetcher{}, sec)
	if _, err := r.resolveWebhookURL(context.Background(), nsDefault,
		&myv1.Webhook{URLSecretRef: &myv1.SecretKeyRef{Name: "wh", Key: secretKeyURL}}); err == nil {
...
func TestResolveWebhookURL_EmptyValue(t *testing.T) {
	scheme := newScheme(t)
	sec := gitAuthSecret("wh", map[string][]byte{secretKeyURL: []byte("   ")}) // whitespace-only
	r := reconcilerFor(scheme, &fakeFetcher{}, sec)
	if _, err := r.resolveWebhookURL(context.Background(), nsDefault,
		&myv1.Webhook{URLSecretRef: &myv1.SecretKeyRef{Name: "wh", Key: secretKeyURL}}); err == nil {
...
func TestResolveWebhookURL_NeitherURLNorRef(t *testing.T) {
	...
	if _, err := r.resolveWebhookURL(context.Background(), nsDefault, &myv1.Webhook{}); err == nil {
		...
		Spec: myv1.DriftCheckSpec{
			Source: myv1.Source{
				...
				Helm: &myv1.HelmSource{
					Git: myv1.GitSource{
						URL:  testRepoURL,
						Auth: &myv1.GitAuth{Type: myv1.GitAuthBasic, SecretRef: myv1.LocalSecretRef{Name: testMissingKey}},
					},
				},
			},
		...
		Spec: myv1.DriftCheckSpec{
			Source: myv1.Source{
				...
				Kustomize: &myv1.KustomizeSource{
					Git: myv1.GitSource{
						URL:  testRepoURL,
						Auth: &myv1.GitAuth{Type: myv1.GitAuthSSH, SecretRef: myv1.LocalSecretRef{Name: testMissingKey}},
					},
				},
			},
			Interval: metav1.Duration{Duration: 5 * time.Minute},
